...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    .setColor('YELLOW')
    .setDescription(`Searching for manga titled **${query}** on :mag_right: [MyAnimeList](https://myanimelist.net 'Homepage').`)
    .setThumbnail('https://i.imgur.com/u6ROwvK.gif')
    .setFooter(`Manga Query with | \©️${new Date().getFullYear()} ${client.user.username}`);
    ...
    for (const res of data.results.slice(0,10)) {
      pages.add(
        ...
        .setFooter([
          ...
          `Manga Query with MAL | \©️${new Date().getFullYear()} ${client.user.username}`
        ].join('\u2000\u2000•\u2000\u2000'))
        .addFields([
