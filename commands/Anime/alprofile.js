  ...
  run: async function ( client, message, args ) {
    ...
    if (response.errors){
      ...
      if (response.errors[0].status === 404){
        ...
        err =`\`${em.error} CLIENT_ERR\`:  ${client.user.username} attempted to send an invalidated request to AniList. Please contact my developer to fix this bug.`;
      } else {
    ...
    return message.channel.send(
      ...
      .setFooter(`ALi Profile | \©️${new Date().getFullYear()} ${client.user.username}`)
