...
module.exports = {
  ...
  run: async ( client, message ) => {
    ...
    .setFooter(`Random Recommendations | \©️${new Date().getFullYear()} ${client.user.username}`);

    // If errored due to ratelimit error
    if (errors && errors.some(x => x.status === 429)){
      return message.channel.send(
        embed.setAuthor(`Oh no! ${client.user.username} has been rate-limited`, 'https://cdn.discordapp.com/emojis/767062250279927818.png?v=1')
        .setDescription([
          `**${message.member.displayName}**, please try again in a minute.\n\n`,
          `If this error occurs frequently, please contact <@340479704962301952>.`
        ].join(''))
      );
    };
    ...
    if (errors && errors.some(x => x.status === 400)){
      return message.channel.send(
        ...
        .setDescription([
          ...
          `Please contact <@340479704962301952> for a quick fix.\n`,
          `You can make an issue on the [repository](${client.config.github}) or [join](https://discord.io/MaskiilovCnD) ${client.user.username}'s dev server instead.`
        ].join(''))
      );
    };
    ...
    if (errors){
      return message.channel.send(
        ...
        .setDescription([
          ...
          `If this doesn't resolve in few hours, you may contact <@340479704962301952>`,
