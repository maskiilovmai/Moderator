  ...
  run: async function (client, message, args ){
    ...
    if (!response || response.status){
      ...
      if (response && response.status >= 500){
        ...
        err = `\`${em.error} CLIENT_ERR\`: ${client.user.username} attempted to send an invalidated request to MAL. Please contact my developer to fix this bug.`
      } else {
    ...
    return message.channel.send(
      new MessageEmbed()
      .setColor(color)
      .setFooter(`Profile | \©️${new Date().getFullYear()} ${client.user.username}`)
