...
module.exports = {
  ...
  run: async function run(client, message, [query]){
    ...
    if (!query){
      ...
      return message.channel.send(`${em.error} | ${message.author}, Please provide me a word to get the definition of.`);
    };
    ...
    if (res.meta.status !== 200){
      return message.channel.send(`${em.error} | ${message.author}, Could not connect to JISHO.`);
    };

    if (!res.data.length){
      return message.channel.send(`${em.error} | ${message.author}, No results were found for **${query}**`);
    };
    ...
    return message.channel.send(
      ...
      .setFooter(`Jisho @ Jisho.org | \©️${new Date().getFullYear()} ${client.user.username}`)
