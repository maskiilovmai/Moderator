...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!data || data.error){
      return message.channel.send(`${em.error} | ${message.author}, I couldn't find the lyrics for ${args.join(' ')}!`)
    };
    ...
    if (data.lyrics.length < 2000){
      return message.channel.send(
        ...
        .setFooter(`Lyrics | \©️${new Date().getFullYear()} ${client.user.username}`)
    ...
    const pages = new Page(
      lyrics_subarray.map((x,i) =>
        ...
        .setFooter([
          ...
          `Lyrics | \©️${new Date().getFullYear()} ${client.user.username}`
        ].join( '\u2000•\u2000' ))
        .setAuthor(`${data.title}\n${data.author}`, null, data.links.genius)
      )
    );
