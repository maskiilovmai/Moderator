...
module.exports = {
  ...
  group: 'Utility',
  description: 'Searches  [Steam](https://store.steampowered.com/ \'Steam Homepage\') for games!, or Doki-doki literature club, if no query is provided.',
  ...
  run: async (client, message, args) => {
    ...
    if (!res || !res.total){
      return message.channel.send(`${em.error} | Could not find **${query}** on ${em.steam} steam`);
    };
    ...
    const platformLogo = { windows: `${em.win}` , mac: `${em.mac}`, linux: `${em.linux}` };
    ...
    return message.channel.send(
      ...
      .setFooter(`Steam @ Steam.Inc©️  | \©️${new Date().getFullYear()} ${client.user.username}`)
      .addFields([
